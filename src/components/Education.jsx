...
const educationData = [
  ...
  },
  {
    ...
    description:
      "In-depth study of computation theory, database management, and operating systems. Achieved academic distinction through rigorous analytical projects.",
  ...
  },
  {
    ...
    grade: "1151 / 1200 (96%)",
    description:
      "Advanced mathematics and computer science track. Graduated as school topper with an exceptional academic record in STEM subjects.",
    ...
    >
      <div className="max-w-6xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-20">
          ...
          >
            Education &{" "}
            <span className="text-transparent bg-clip-text bg-linear-to-r from-indigo-500 to-teal-400">
              Excellence.
            </span>
          </motion.h2>
        </div>
            ...
            {educationData.map((item, index) => (
              <div key={item.id} className="relative">
                ...
                >
                  ...
                  >
                    <div className="glass-panel p-8 rounded-4xl hover:border-indigo-500/30 transition-all duration-300 relative overflow-hidden backdrop-blur-sm">
                      {/* Hover Glow */}
                      <div
                        className={`absolute -right-10 -top-10 w-32 h-32 bg-linear-to-br ${item.color} opacity-0 group-hover:opacity-10 dark:group-hover:opacity-20 blur-3xl transition-opacity duration-500`}
                      />

                      <div className="flex items-start justify-between mb-6">
                        <div
                          className={`w-12 h-12 rounded-2xl bg-linear-to-br ${item.color} flex items-center justify-center text-white shadow-lg transition-transform duration-500 group-hover:rotate-12`}
                        >
                          {item.icon}
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2 pt-6 border-t border-slate-100 dark:border-slate-800">
                        {item.tags.map((tag, idx) => (
                          <span
                            key={idx}
                            className="px-2.5 py-1 bg-slate-50 dark:bg-slate-800/80 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider rounded-lg border border-slate-200 dark:border-slate-700/50 group-hover:border-indigo-500/20 transition-all"
                          >
                            {tag}
                          </span>
                        ))}
